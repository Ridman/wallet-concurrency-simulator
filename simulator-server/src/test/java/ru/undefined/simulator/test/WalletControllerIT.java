...
import ru.undefined.simulator.commons.Requests;
...
public class WalletControllerIT extends AbstractIT {
    ...
    public void whenInsufficientFundsThenAppropriateMessage() {
        ...
        walletController.withdraw(transactionBody(user, 200, Currency.USD),
                insufficientFundsValidator());
        // 2. Make a deposit of USD 100 to user with id 1.
        walletController.deposit(transactionBody(user, 100, Currency.USD), okValidator());
        ...
        walletController.deposit(transactionBody(user, 100, Currency.EUR), okValidator());
        ...
        walletController.withdraw(transactionBody(user, 200, Currency.USD), okValidator());
    ...
    private TransactionBody transactionBody(User user, double amount, Currency currency) {
        return Requests.transactionBody(user.getId(), amount, currency.name());
    }

    private BalanceRequest balanceRequest(User user) {
        return Requests.balanceRequest(user.getId());
    }
}
